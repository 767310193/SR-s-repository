...
for url_product in quallist:
   ...
   # response_product = request.urlopen(rps_product)
    ...
    # f.write(str(html_1))
    # f.close()
    insert_txt="INSERT INTO `ebay_of_keyword_camera+cage` VALUES('"+str(product_name)+"',"+str(sold_num)+")"
    update_txt="UPDATE `ebay_of_keyword_camera+cage` SET sold_num = "+str(sold_num)+" WHERE product_name = '"+str(product_name)+"'"
    while 1:
        try :
...
while counter < 200:
    #print(counter)
    url = "http://www.ebay.com/sch/i.html?_from=R40&_sacat=0&_sop=10&_nkw=camera+cage&_pgn="+str(counter)+"&_skc=50&rt=nc"
    print(url)
    # url="http://www.ebay.com/sch/i.html?_odkw=camera+cage&_osacat=0&_from=R40&_trksid=p2045573.m570.l1313.TR0.TRC0.H0.TRS0&_nkw=camera+cage&_sacat=0"
    ###########获取当页所有产品链接
    rps = request.Request(url)
    ...
    for url_product in quallist:
        ...
        # f.write(str(html_1))
        # f.close()
        insert_txt = "INSERT INTO `ebay_of_keyword_camera+cage` VALUES('" + str(product_name) + "'," + str(
            sold_num) + ")"
        update_txt = "UPDATE `ebay_of_keyword_camera+cage` SET sold_num = " + str(
            sold_num) + " WHERE product_name = '" + str(product_name) + "'"
        while 1:
            try:
                cursor.execute(insert_txt)
            except (pymysql.err.InterfaceError,pymysql.err.ProgrammingError):
